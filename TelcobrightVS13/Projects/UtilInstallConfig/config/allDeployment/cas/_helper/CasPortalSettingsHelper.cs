...
{
    public class CasPortalSettingsHelper
    {
        public static PortalSettings GetCasCommonPortalSettings(TelcobrightConfig tbc)
        {
            ...
            PortalSettings portalSetting = new PortalSettings("Portal Settings")
            {
                ...
                {
                    new InternetSite(tbc)//make sure that first one always the http portal
                    {
                        SiteType = "http",
                        SiteName = tbc.Telcobrightpartner.CustomerName,
                        SiteId = 1,
                        PhysicalPath = "C:/inetpub/wwwroot/" + tbc.Telcobrightpartner.CustomerName,
                        BindAddress = "0.0.0.0:80",
                        TemplateFileName = "../../" + tbc.Telcobrightpartner.CustomerName + "/tmplPortalWebSite.txt",
                        ApplicationPool=new IisApplicationPool()
                        {
                            AppPoolName = tbc.Telcobrightpartner.CustomerName,
                            TemplateFileName = "../../" + tbc.Telcobrightpartner.CustomerName + "/tmplPortalAppPools.txt",
                        },
