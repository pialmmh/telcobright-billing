...
namespace Decoders
{
    ...
    public class TelcobridgeDecoderGaziZip : AbstractCdrDecoder
    {
            ...
            out List<cdrinconsistent> inconsistentCdrs)
        {
            ...
            lock (threadLocker)
            {
                ...
                lines = linesReader.readLinesFromCompressedFile();
            }
            return DecodeLine(input, out inconsistentCdrs, fileName, lines);
        }

        private  List<string[]> DecodeLine(CdrCollectorInputData input,
            out List<cdrinconsistent> inconsistentCdrs, string fileName, string [] lines)
        {
            this.Input = input;
            //string fileName = this.Input.FullPath;
           ...
           // string[] linesAsString = File.ReadAllLines(fileName);
            ...
            try
            {
                foreach (string ln in lines)
                {
                    ...
                    string startTime = lineAsArr[1].Trim();
