...
namespace Decoders
{
    ...
    public class DialogicBorderNetMirIgwNoFailed : AbstractCdrDecoder
    {
        public override string ToString() => this.RuleName;
        public override string RuleName => GetType().Name;
        public override int Id => 234;
        public override string HelpText => "Decodes Dialogic BorderNet CSV CDR (Mir Telecom (IGW))";
            ...
            try
            {
                foreach (string[] lineAsArr in lines)
                {
                    ...
                    if (ingressRequestLine.IsNullOrEmptyOrWhiteSpace() == false)
                    {
                        ...
                        // incoming route
                        textCdr[Fn.IncomingRoute] = originatingIp.Split(':')[0];
                    ...
                    if (outSigReqLine.IsNullOrEmptyOrWhiteSpace() == false)
                    {
                        ...
                        // outgoing route
                        textCdr[Fn.OutgoingRoute] = terminatingIp.Split(':')[0];
