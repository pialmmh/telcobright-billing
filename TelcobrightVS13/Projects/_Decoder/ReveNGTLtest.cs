...
namespace Decoders
{
    ...
    public class ReveNGTLtest : IFileDecoder
    {
        ...
        protected static List<string[]> decodeLines(CdrCollectorInputData input, out List<cdrinconsistent> inconsistentCdrs, string fileName, List<string[]> lines)
        {
            ...
            foreach (string[] lineAsArr in lines)
            {
                ...
                string startTimestr = lineAsArr[9].Trim();
                string answerTimeStr = lineAsArr[10].Trim();
                ...
                string originatingIp = lineAsArr[4];
                string originatingPort = lineAsArr[5];
                ...
                textCdr[Fn.Mediaip1] = lineAsArr[15];
                textCdr[Fn.Mediaip2] = lineAsArr[17];

                if (!string.IsNullOrEmpty(lineAsArr[6].Trim())) textCdr[Fn.OriginatingCalledNumber] = lineAsArr[7].Trim();
                if (!string.IsNullOrEmpty(lineAsArr[8].Trim())) textCdr[Fn.OriginatingCallingNumber] = lineAsArr[9].Trim();
                if (!string.IsNullOrEmpty(lineAsArr[7].Trim())) textCdr[Fn.TerminatingCalledNumber] = lineAsArr[8].Trim();
                if (!string.IsNullOrEmpty(lineAsArr[8].Trim())) textCdr[Fn.TerminatingCallingNumber] = lineAsArr[9].Trim();
