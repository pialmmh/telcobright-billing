...
namespace Process
{
    ...
    public class MdrJobCreator : AbstractTelcobrightProcess
    {
        ...
        public override string HelpText => "Method to Create Mdr Job";
        public override int ProcessId => 111;
        ...
        public override void Execute(IJobExecutionContext schedulerContext)
        {
            ...
            Console.WriteLine($"MdrJobCreater {operatorName}");
            try
            {
                ...
                foreach (ne thisSwitch in context.nes.Where(c => c.idCustomer == idOprator).ToList())
                {
                    try
                    {
                        ...
                        cdrSetting.NeWiseAdditionalSettings.TryGetValue(thisSwitch.idSwitch, out neAdditionalSetting);
                        Dictionary<string, FileInfo> newJobNameVsFileInfos = new Dictionary<string, FileInfo>();

                        if (neAdditionalSetting.CreateJobRecursively == false)
                        {
                            fileInfos = dirlister.ListLocalDirectoryNonRecursive(vaultPath)
                                .Where(fInfo =>
                                {
                            ...
                            newJobNameVsFileInfos = fileInfos
                        ...
                        {
                            dirlister.ListLocalFileRecursive(vaultPath, fileInfos);
                            fileInfos = fileInfos
                                .Where(fInfo =>
                                {
                                    if (thisSwitch.FileExtension == null)
                            ...
                            newJobNameVsFileInfos = fileInfos
                                .Select(f => // kv<jobName,fileName>
                                    {
                                        string jobName = prepareRecursiveJobName(vaultPath, f.FullName);
                                        return new
                                        {
                                            jobName = jobName,
                                            fileName = f
                        ...
                        foreach (FileInfo fileInfo in fileInfos)
                        {
                                ...
                                fileInfo.Length <= fileSplitSetting.SplitFileIfSizeBiggerThanMbyte)
                            {
                                ...
                                {
                                    string jobname = prepareRecursiveJobName(vaultPath, fileInfo.FullName);
                                    job newJob = CreateRecursiveSingleCdrJob(context, thisSwitch, fileInfo, jobname);
                                    newJobCacheForWritingAtOnceToDB.Add(newJob);
                                }
                    ...
                    catch (Exception e1)
                    {
                        ...
                        ErrorWriter.WriteError(e1, "MdrJobCreator/SwitchId:" + thisSwitch.idSwitch, null, "",
            ...
            {
                Console.WriteLine(e1);
                ErrorWriter.WriteError(e1, "MdrJobCreator", null, "", operatorName, context);
            }
        }
            ...
            int existingCount)
        {
            ...
            {
                Console.WriteLine(" No new mdr file found, exiting...");
            }
            Console.WriteLine("Writing " + newJobCacheForWritingAtOnceToDB.Count + " mdr jobs to db...");
            context.jobs.AddRange(newJobCacheForWritingAtOnceToDB);
            context.SaveChanges();
            Console.WriteLine("switch: " + thisSwitch.SwitchName + ", new mdr job created:"
        ...
        private static job CreateRecursiveSingleCdrJob(PartnerEntities context, ne thisSwitch, FileInfo fileInfo,
            string jobName)
        {
        ...
        private static string prepareRecursiveJobName(string vaultPath, string fileName)
        {
            if (fileName.Contains("\\"))
            {
                fileName = fileName.Replace("\\", "/");
            }
            int startingPathLen = vaultPath.Length;
            string relativeFileName = fileName.StartsWith(vaultPath) ?
