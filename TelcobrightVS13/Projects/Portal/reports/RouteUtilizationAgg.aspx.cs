...
public partial class DefaultRtUtilAgg : System.Web.UI.Page
{
    ...
    {   
        ...
        using (MySqlConnection connection = new MySqlConnection())
        {
            ...
            {
                cmd.CommandText = "CALL ErlangByRouteAggAll(@p_StartDateTime,@p_EndDateTime,@p_W)";
                ...
                if (idRoute > 0)
                {
                    using (PartnerEntities context = new PartnerEntities())
                    {
                        ...
                        if (thisRoute != null)
                        {
                            ...
                            {
                                cmd.CommandText = "CALL ErlangByRouteAggOne(@p_StartDateTime,@p_EndDateTime,@p_W,@p_RouteName,@p_SwitchId)";
            ...
            this.Session["RouteUtilizationboth.aspx.csdt260"] = colNamelist;//THIS MUST BE CHANGED FOR EACH PAGE
            //now add sort order to override the column list in the dataset and to match gridview setting


            this.Session["RouteUtilizationboth.aspx.csdt360"] = columnSortList;
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        List<string> colNameList=new List<string>();
        List<int> columnSortList=new List<int>();
        if (this.Session["RouteUtilizationboth.aspx.csdt16"] != null) //THIS MUST BE CHANGED IN EACH PAGE
        {
            dt2 = (DataTable) this.Session["RouteUtilizationboth.aspx.csdt16"];//THIS MUST BE CHANGED IN EACH PAGE
            
            if (this.Session["RouteUtilizationboth.aspx.csdt260"] != null) //THIS MUST BE CHANGED IN EACH PAGE
            {
                colNameList = (List<string>) this.Session["RouteUtilizationboth.aspx.csdt260"];//THIS MUST BE CHANGED IN EACH PAGE
            }

            if (this.Session["RouteUtilizationboth.aspx.csdt360"] != null) //THIS MUST BE CHANGED IN EACH PAGE
            {
                columnSortList = (List<int>) this.Session["RouteUtilizationboth.aspx.csdt360"];//THIS MUST BE CHANGED IN EACH PAGE
            }
