...
public partial class DefultMonthlyRomingReport: System.Web.UI.Page
{
    ...
    List<SumDataOfMonth> ConvertBtrcDataSetToList(DataSet ds) {
        ...
        if (hasRecords == true)
        {
            foreach (DataTable table in ds.Tables)
            {
                ...
                {
                    SumDataOfMonth record = new SumDataOfMonth();
                    record.startTime = row.Field<DateTime>("Start Time").ToString();
                    record.sourceNetwork = row.Field<string>("Source Network");
                    record.destinationNetwork = row.Field<string>("Destination Network");
                    record.callerNumberA = row.Field<string>("Caller Number (A NUM)");
                    record.calledNumberB = row.Field<string>("Called Number (B Num)");
                    record.redirectingNumber = row.Field<string>("Redirect Number");
                    record.billedDuration = row.Field<decimal>("Billed Duration").ToString();
    ...
    DataSet getMonthlyReport(MySqlConnection connection ,string startDate,string endDate)
    {
                string Sql = $@"select starttime as `Start Time`,cr.partnername as 'Source Network',
                    ...
                    where starttime>='{startDate} 00:00:00'
                    ...
                    and originatingcallingnumber not like '0096%'
                    and originatingcallingnumber not like '96%'
                    and originatingcallingnumber not like '096%'
                    and originatingcallingnumber not like '88096%'
                    )
                    order by starttime
                    limit 0,1000000;";
    ...
    protected void submit_Click(object sender, EventArgs e)
    {
        ...
        using (MySqlConnection connection = new MySqlConnection())
        {
            ...
            if (int.TryParse(yearStr, out yearValue))
            {
                ...
                int daysInMonth = DateTime.DaysInMonth(yearValue, startDate.Month);

                for (int day = 1; day <= daysInMonth; day++)
                {
                    DateTime date = new DateTime(yearValue, startDate.Month, day);
                    listOfdate.Add(date);
                }
            }
       ...
       // List<MonthlyReportRowForExcel> monthlyReport= new List<MonthlyReportRowForExcel>();
        ...
        using (MySqlConnection connection = new MySqlConnection())
        {
            ...
            ExcelExporterForBtrcReport.ExportToExcelMonthlyRoamingReport("MonthlyReport From " + txtStartDate.Text + " To " + txtEndDate.Text
                    + ".xlsx", Response, sumOfMonthlyReport, this.operatorName);
