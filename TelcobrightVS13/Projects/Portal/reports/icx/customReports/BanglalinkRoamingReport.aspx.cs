...
public partial class BanglalinkRoamingReport : System.Web.UI.Page
{
    ...
    List<customBlCallRoaming> ConvertBtrcDataSetToList(DataSet ds) {
        bool hasRecords = ds.Tables.Cast<DataTable>()
                           .Any(table => table.Rows.Count != 0);
        //List<MonthlyReportRow> records = new List<MonthlyReportRow>();
        List<customBlCallRoaming> finalRecords= new List<customBlCallRoaming>();
        if (hasRecords == true)
        {
            foreach (DataTable table in ds.Tables)
            {
                ...
                {
                    customBlCallRoaming record = new customBlCallRoaming();
                    //record.SLNo = Slno;
                    //Slno += 1;

                    record.startTime = row.Field<DateTime>("Start Time");
                    record.SourceNetwork = row.Field<string>("Source Network");
                    record.DestinationNetwork = row.Field<string>("Destination Network");
                    record.CallerNumber_ANUM_ = row.Field<Decimal>("Called Number (A Num)");
                    record.CalledNumber_BNum_ = row.Field<Decimal>("Called Number (B Num)");                    
                    record.RedirectNumber = row.Field<Decimal>("Redirect Number");
                    record.BilledDuration = row.Field<int>("Billed Duration");
    ...
    protected void submit_Click(object sender, EventArgs e)
    {
        ...
        List<customBlCallRoaming> monthlyRecords = new List<customBlCallRoaming>();
        ...
        using (MySqlConnection connection = new MySqlConnection())
        {
            ...
            if (hasdomesticDs == true)
            {
                ...
                DomHeader.Text = "custom Banglalink Call Roaming";
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        List<customBlCallRoaming> monthlyReport= new List<customBlCallRoaming>();
        ...
        using (MySqlConnection connection = new MySqlConnection())
        {
           ...
           ExcelExporterForBtrcReport.ExportToExcelBanglalinkRoamingReport("MonthlyReport From " + txtStartDate.Text + " To " + txtEndDate.Text
