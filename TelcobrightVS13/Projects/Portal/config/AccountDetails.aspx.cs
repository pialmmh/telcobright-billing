...
public partial class AccountDetails : System.Web.UI.Page
{
    ...
    protected void gvAccount_OnRowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            GridView gvAccount = sender as GridView;
            int id = Convert.ToInt32(gvAccount.DataKeys[e.Row.RowIndex].Value);
            Label lblAutomation = e.Row.FindControl("lblAutomation") as Label;

            using (PartnerEntities context = new PartnerEntities())
            {
                string automation = string.Empty;
                List<acc_action> actions = context.acc_action.Where(x => x.idAccount == id).OrderByDescending(x => x.threshhold_value).ToList();
                foreach (acc_action action in actions)
                {
                    automation += $"{availableActions.First(x => x.Id == action.idAccountAction).ActionName}@{Math.Round(action.threshhold_value, 2)}, ";
                }
