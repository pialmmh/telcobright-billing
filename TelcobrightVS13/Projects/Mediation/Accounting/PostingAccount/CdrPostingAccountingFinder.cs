    ...
    public class CdrPostingAccountingFinder
    {
        ...
        account GetAssignDirWiseReceivableOrPayableAccount(BillingRule billingRule)
        {
            ...
            if (this.ServiceContext.AssignDir== ServiceAssignmentDirection.Customer)
            {
                ...
                {
                    postingAccount = accountFactory.CreateOrGetCustomerAccount(0, this.ServiceContext.IdServiceGroup, this.IdChargedOrChargingPartner
                        , this.ServiceContext.ServiceFamily.Id, this.ServiceContext.ProductIdToOverrideServiceFamilyAccount, idCurrencyUoM);
                }
                else//postpaid                                                                                                
                {
                    postingAccount = accountFactory.CreateOrGetBillable(0, this.ServiceContext.IdServiceGroup, this.IdChargedOrChargingPartner
                        , this.ServiceContext.ServiceFamily.Id, this.ServiceContext.ProductIdToOverrideServiceFamilyAccount, idCurrencyUoM);
                }
            }
            if (this.ServiceContext.AssignDir == ServiceAssignmentDirection.Supplier)
            {
                if (billingRule.IsPrepaid)//prepaid                                                                           
                {
                    postingAccount = accountFactory.CreateOrGetSupplierAccount(0, this.ServiceContext.IdServiceGroup, this.IdChargedOrChargingPartner
                        , this.ServiceContext.ServiceFamily.Id, this.ServiceContext.ProductIdToOverrideServiceFamilyAccount, idCurrencyUoM);
                }
                else//postpaid                                                                                                
                {
                    postingAccount = accountFactory.CreateOrGetPayable(0, this.ServiceContext.IdServiceGroup, this.IdChargedOrChargingPartner
                        , this.ServiceContext.ServiceFamily.Id, this.ServiceContext.ProductIdToOverrideServiceFamilyAccount, idCurrencyUoM);
                }
            }
            return postingAccount;
