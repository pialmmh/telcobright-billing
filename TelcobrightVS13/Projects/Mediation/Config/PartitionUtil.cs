...
namespace TelcobrightMediation.Config
{
    public static class PartitionUtil
    {
        public static void ModifyPartitions(DatabaseSetting databaseSetting, string operatorName)
        {
            ...
            {
                con.Open();
                Dictionary<string, List<PartitionInfo>> allPartitions = getAllPartitionsInfo(con, databaseSetting)
        ...
        public static List<PartitionInfo> getAllPartitionsInfo(MySqlConnection con, DatabaseSetting databaseSetting)
        {
            using (MySqlCommand cmd = new MySqlCommand("", con))
            {
                cmd.CommandText = $@"select 
                                        ...
                                        where TABLE_SCHEMA='{databaseSetting.DatabaseName}'
                                        and table_name not like 'lcr%'
                                        and table_name not like 'rate%') t
                                        on p.table_name=t.table_name
                                        where p.TABLE_SCHEMA='{databaseSetting.DatabaseName}'
