...
namespace RateSheetFormat
{
    ...
    public class RsDblNormalized : IRateSheetFormat//change here
    {
        public override string ToString()
        {
            return this.RuleName;
        }
        private string _helpText = "Rate Sheet Format for Dbl Normalized";//change for each extension
        public string RuleName
        {
            get { return "rsFormatDblNormalized"; }//change for each extension
        }
        ...
        public int Id
        {
            get { return 12; }//change for each extension
        }
        ...
        public RsDblNormalized()//change here
        {
            //
        ...
        List<ratetask> ProcessRateSheet(Worksheet sheet1,SheetImportTask importTask1,rateplan rp, string[] dateFormats,ref string dateSeparator)
        {
            Range range1;
            ...
            while (true)
            {
                if (i > dim11) break;
                ratetask thisTask = new ratetask();
                thisTask.Prefix = (objArray1[i, 2] == null ? "" : objArray1[i, 2].ToString().Trim());
                if (thisTask.Prefix != "")
                {
                    thisTask.description = (objArray1[i, 1] == null ? "" : objArray1[i, 1].ToString());
                    thisTask.MinDurationSec = "0";
                    thisTask.Resolution = "1";
                    double rateAmount = 0;
                    if(double.TryParse((objArray1[i, 3] == null ? "" : objArray1[i, 3].ToString()),out rateAmount)== true)
                    {
                        thisTask.rateamount = rateAmount.ToString();
                    }
                    string strTempDate = (objArray1[i, 4] == null ? "" : objArray1[i, 4].ToString());
                    //for performance 
                    DateTime? tempDate1 = (objArray1[i, 4] == null ? new DateTime(1,1,1,0,0,0) : (DateTime)objArray1[i, 4]);
                    DateTime tempDate = new DateTime(1, 1, 1, 0, 0, 0);
                    if (tempDate1!=new DateTime(1,1,1,0,0,0))
                    {
                        thisTask.startdate = (Convert.ToDateTime(tempDate1)).ToString("yyyy-MM-dd HH:mm:ss");
                    }
                    else if (DateTime.TryParseExact(strTempDate, dateFormats, CultureInfo.InvariantCulture, DateTimeStyles.None, out tempDate))
                    {
                        thisTask.startdate = tempDate.ToString("yyyy-MM-dd HH:mm:ss");
                    }
        ...
        public string GetRates(string filePath, ref List<ratetask> lstRateTask, rateplan rp, bool endAllPrevPrefix, string[] dateFormats)
        {
            //try
            {
                ...
                lstRateTask =ProcessRateSheet(sheet,importTask,rp,dateFormats,ref dateSeparator);
