...
namespace RateSheetFormat
{
    ...
    public class RsSingtel : IRateSheetFormat//change here
    {
        public override string ToString()
        {
            return this.RuleName;
        }
        private string _helpText = "Rate Sheet Format for Singtel";//change for each extension
        public string RuleName
        {
            get { return "rsFormatSingtel"; }//change for each extension
        }
        ...
        public int Id
        {
            get { return 8; }//change for each extension
        }
        ...
        {
            Range range1 = sheet1.UsedRange;
            Range range2 = sheet2.UsedRange;
            
            object[,] objArray1 = (object[,])range1.get_Value(XlRangeValueDataType.xlRangeValueDefault);
            object[,] objArray2 = (object[,])range2.get_Value(XlRangeValueDataType.xlRangeValueDefault);
            ...
            //create description based dictionary from first sheet
            Dictionary<string, ratetask> dicRateTask1 = new Dictionary<string, ratetask>();
            int i = firstRow1;
            while (true)
            {
                ...
                {
                    if (dicRateTask1.ContainsKey(thisTask.description))
                    {
                        i++;
                        continue;
                    }
                        
                    thisTask.rateamount = (objArray1[i, 4] == null ? "" : objArray1[i, 4].ToString());
                    thisTask.startdate = (objArray1[i, 6] == null ? "" : objArray1[i, 6].ToString());
                    dicRateTask1.Add(thisTask.description,thisTask);
                    i++;
                }
                else
                {
            ...
            //retrieve cc+area code + description from 2nd sheet
            List<ratetask> lstRateTask2 = new List<ratetask>();
            i = firstRow2;
            while (true)
            {
                ratetask thisTask = new ratetask();
                thisTask.description = (objArray2[i, 2] == null?"": objArray2[i, 2].ToString());
                if (thisTask.description.Trim() != "")
                {
                    if (dicRateTask1.ContainsKey(thisTask.description) == true)
                    {//rate and date has to be found based on description, add to list only if key found in the dic
                        thisTask.CountryCode = objArray2[i, 3] == null ? "" : objArray2[i, 3].ToString();
                        thisTask.Prefix = objArray2[i, 3].ToString() + (objArray2[i, 4] == null ? "" : objArray2[i, 4].ToString());
                        thisTask.rateamount = dicRateTask1[thisTask.description].rateamount;
                        string strTempDate = dicRateTask1[thisTask.description].startdate;
        ...
        public string GetRates(string filePath, ref List<ratetask> lstRateTask, rateplan rp, bool endAllPrevPrefix, string[] dateFormats)
        {
            //try
            {
                ...
                //for singtel , supersede all prefix
                endAllPrevPrefix = true;
                ...
                if (endAllPrevPrefix == true)
                {
                    ...
                    delTask.Prefix = "*";
                    delTask.description = "End All Codes";
                    delTask.rateamount = "-1";//rate
                    DateTime delDate = ((Range)sheet2.Cells[3, 2]).Value == null ? DateTime.Now : ((Range)sheet2.Cells[3, 2]).Value;
                    delTask.startdate = delDate.ToString("yyyy-MM-dd HH:mm:ss");
