...
namespace RateSheetFormat
{
    ...
    public class RsDialogAxiata : IRateSheetFormat//change here
    {
        public override string ToString()
        {
            return this.RuleName;
        }
        private string _helpText = "Rate Sheet Format for DialogAxiata";//change for each extension
        public string RuleName
        {
            get { return "rsFormatDialogAxiata"; }//change for each extension
        }
        ...
        public int Id
        {
            get { return 6; }//change for each extension
        }
        ...
        public RsDialogAxiata()//change here
        {
            //
                ...
                try
                {
                    foreach (string[] thisRow in lstSinglePrefixArray)
                    {
                        ...
                        if (this._importTaskType == ImportTaskType.RateChanges)
                        {
                            ...
                            {
                                //dialog gives both 1 & 2 columns for rates
                                newTask.rateamount = thisRow[this.VendorParams.ParamDialog.RateColIndex];
                                double tempDouble = 0;
                                if (double.TryParse(newTask.rateamount,out tempDouble)==false)
                                {
                                    newTask.rateamount = thisRow[tableData.IndexRate - 1];
                                }
                            }
                            else
                            {
