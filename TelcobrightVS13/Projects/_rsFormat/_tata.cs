...
namespace RateSheetFormat
{
    ...
    public class RsTata : IRateSheetFormat
    {
        public override string ToString()
        {
            return this.RuleName;
        }
        private string _helpText = "Rate Sheet Format for Tata";//change for each extension
        public string RuleName
        {
            get { return "rsFormatTata"; }//change for each extension
        }
        ...
        public int Id
        {
            get { return 3; }//change for each extension
        }
        ...
        public string GetRates(string filePath, ref List<ratetask> lstRateTask, rateplan rp, bool endAllPrevPrefix, string[] dateFormats)
        {
            //try
            {
                ...
                else//code delete all =false
                {
                    if (importTask.VFormat == VendorFormat.Tata)
                    {
                        sheet = (Worksheet)book.Worksheets[2];
                        SheetImportTask deleteTask = new SheetImportTask(ImportTaskType.CodeChanges, sheet, this.DicCountryCode, this.DicCountryName, rp);
                        //vendor format already determined
                        deleteTask.VFormat = VendorFormat.Tata;
                        retval = deleteTask.GetRateArray(ref lstCodeDeletes, rp, book, dateFormats, ref dateSeparator);
                    }

                    else if (importTask.VFormat == VendorFormat.Bharti)
                    {
                        List<rate> lstOldOpenRates = new List<rate>();
                        Dictionary<string, rate> dicNewCodes = new Dictionary<string, rate>();
                        ...
                        foreach (ratetask thisTask in lstRateTask)
                        {
                            ...
                            if (dicNewCodes.ContainsKey(newRate.Prefix) == false)
                                dicNewCodes.Add(newRate.Prefix, newRate);
                        }

                        //find out new code vs old code differences, first column is prefix in strlines
        ...
        public class SheetImportTask
        {
            ...
            public string GetRateArray(ref List<ratetask> lstRateTask, rateplan rp, Workbook book, string[] dateFormats, ref string dateSeparator)
            {
                try
                {
                    ...
                    //if there is no code delete sheet then below will throw exception
                    try
                    {
                        objArray = (object[,]) this._range.get_Value(XlRangeValueDataType.xlRangeValueDefault);
                    }
                    catch(Exception e)
                    {
                    ...
                    if (this.FirstRow == 2)//there is coumn header
                    {
                    ...
                    if (this._importTaskType == ImportTaskType.RateChanges)//vendor format already set after rate changes sheet processing
                    {
                        this.VFormat = GetVendorFormat(ref objArray, this.FirstRow, book);
                    }
                    else
                    {
                    ...
                    FindTableMetaData(ref objArray, ref this._tableData, this.FirstRow, this.VFormat, dateFormats, ref dateSeparator);
