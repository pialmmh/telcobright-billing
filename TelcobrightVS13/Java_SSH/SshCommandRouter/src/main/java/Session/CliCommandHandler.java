...
public class CliCommandHandler {
    ...
    public CliCommandHandler(ISessionInfo sessionInfo,int maxRetryForCommand,String moreOutputPendingIndicator,
                             String strToSendForMoreOutput) {//constructor
    ...
    private List<String> executeCommandWithMoreChecking(SingleCliCommand singleCliCommand) throws Exception {
        ...
        if(!output.isEmpty())
        {
            ...
            while(moreExists=true)
            {
                ...
                if(lastChunk.contains(this.moreOutputPendingIndicator)==false
                ...
                output.set(output.size()-1,output.get(output.size()-1).replace(this.moreOutputPendingIndicator,""));
                List<String> moreOutput=executeCommand(this.strToSendForMoreOutput);
