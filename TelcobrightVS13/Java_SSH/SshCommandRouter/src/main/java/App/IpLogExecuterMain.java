...
public class IpLogExecuterMain {
    ...
    public static void main(String[] args) {
        try {
            ...
            String output = getOutputFromSession(context);
    ...
    static String getOutputFromSession(ClassPathXmlApplicationContext context) throws Exception {
        ...
        commandSequence.init("--More-- ", " ");
        List<String> output = new ArrayList<String>();
        for (SingleCliCommand singleCliCommand : commandSequence.getCommands()) {
            CliCommandHandler commandHandler = new CliCommandHandler(sessionInfo, 3,
                    commandSequence.getMoreOutputPendingIndicator(),
                    commandSequence.getStrToSendForMoreOutput());

            output = commandHandler.sendCommand(singleCliCommand);

            if (sessionInfo.getCurrentPrompt() != singleCliCommand.getExpectedPromptAfter()) {
